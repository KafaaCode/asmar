                    ...
                    <div class="d-flex align-items-center">
                        <div class="flex-shrink-0">
                            ...
                            @if (auth()->user()->hasPermission('update_users'))
                                <form action="{{ route('ad.users.resetBalances') }}" method="POST" style="display:inline;"
                                    id="withdrawForm">
                                    @csrf
                                    ...
                                    @foreach ($users as $index => $user)
                                        <tr>
                                            ...
                                            <th scope="row">
                                                ...
                                                <button class="btn btn-sm {{ $userDiscount ? 'btn-success' : 'btn-info' }} mt-2"
                                                    data-bs-toggle="modal" data-bs-target="#discountModal{{ $user->id }}">
                                                    {{ $userDiscount ? 'خصم: ' . $userDiscount->amount . '%' : 'إضافة خصم' }}
                                                </button>

                                                <div class="modal fade" id="discountModal{{ $user->id }}" tabindex="-1"
                                                    aria-labelledby="discountModalLabel{{ $user->id }}" aria-hidden="true">
                                                    <div class="modal-dialog">
                                                        <form
                                                            action="{{ $userDiscount ? route('ad.discounts.update', $userDiscount->id) : route('ad.discounts.store') }}"
                                                            method="POST">
                                                            @csrf
                                                            @if($userDiscount)
                                                                @method('PUT')
                                                            ...
                                                            <div class="modal-content">
                                                                <div class="modal-header">
                                                                    <h5 class="modal-title" id="discountModalLabel{{ $user->id }}">
                                                                        {{ $userDiscount ? 'تعديل خصم' : 'إضافة خصم' }} للمستخدم:
                                                                        {{ $user->name }}
                                                                    </h5>
                                                                ...
                                                                <div class="modal-body">
                                                                    <div class="mb-3">
                                                                        <label for="amount{{ $user->id }}" class="form-label">نسبة
                                                                            الخصم (%)</label>
                                                                        <input type="number" name="amount"
                                                                            id="amount{{ $user->id }}" class="form-control"
                                                                            step="0.01" min="0" required
                                                                            value="{{ $userDiscount ? $userDiscount->amount : '' }}">
                                                                    </div>
                                                                </div>
